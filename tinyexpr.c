...
static double equal(double a, double b) { return fabs(a - b) < 1e-9 ? 1.0 : 0.0; }
static double not_equal(double a, double b) { return fabs(a - b) > 1e-9 ? 1.0 : 0.0; }
static double less(double a, double b) { return a < b ? 1.0 : 0.0; }
static double less_eq(double a, double b) { return a <= b ? 1.0 : 0.0; }
static double greater(double a, double b) { return a > b ? 1.0 : 0.0; }
static double greater_eq(double a, double b) { return a >= b ? 1.0 : 0.0; }
        ...
        if ((s->next[0] >= '0' && s->next[0] <= '9') || s->next[0] == '.') {
            ...
            if (isalpha(s->next[0])) {
                ...
                switch (s->next++[0]) {
                    ...
                    case '=':
                        if (s->next[0] == '=') { s->next++; s->type = TOK_INFIX; s->function = equal; }
                        else s->type = TOK_ERROR;
                        break;
                    case '!':
                        if (s->next[0] == '=') { s->next++; s->type = TOK_INFIX; s->function = not_equal; }
                        else s->type = TOK_ERROR;
                        break;
                    case '<':
                        if (s->next[0] == '=') { s->next++; s->type = TOK_INFIX; s->function = less_eq; }
                        else { s->type = TOK_INFIX; s->function = less; }
                        break;
                    case '>':
                        if (s->next[0] == '=') { s->next++; s->type = TOK_INFIX; s->function = greater_eq; }
                        else { s->type = TOK_INFIX; s->function = greater; }
                        break;
...
static te_expr* relational(state* s) {
    ...
    while (s->type == TOK_INFIX &&
        (s->function == equal || s->function == not_equal ||
            s->function == less || s->function == less_eq ||
